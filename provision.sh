...
# Create a symlink of default site in enabled folder
...
# Download and Install PHP 7 PDO drivers for MsSQL server for linux

cd ~
git clone https://github.com/Azure/msphpsql.git --branch PHP-7.0-Linux --single-branch
cp msphpsql/Ubuntu1504/*.so /usr/lib/php/
sed -i "839i extension=/etc/php/lib/php_pdo_sqlsrv_7_ts.so" /etc/php/7.0/fpm/php.ini
